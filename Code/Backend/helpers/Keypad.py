...
class Keypad(threading.Thread):
    def __init__(self, message_queue, lcd, muntstuk_acceptor, load_cell):
    ...
    def handle_key_press(self, key):
        if key == 'A' and len(self.input_string) > 0:
            product = DataRepository.get_product_by_number(int(self.input_string))
            self.input_string = ""
            ...
            if not self.ip_shown:
                ...
                if len(self.input_string) > 0:
                    self.muntstuk_acceptor.accepting_coins = False
                    self.lcd.write_message("Keuze Product:", LCD_LINE_1, clear=True)
                    self.lcd.write_message(self.input_string, LCD_LINE_2)
                    self.ip_shown = False
                else:
                    self.muntstuk_acceptor.accepting_coins = True
                    self.lcd.display_total_credit(self.muntstuk_acceptor.money_paid)
                    self.ip_shown = False
